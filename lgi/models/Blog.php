...
class BlogModel{
	public function getOne($value, $field = "id"){
		try {
			...
			return $entityManager->getRepository("Blog")->findOneBy(array($field => $value));
		} catch (Exception $e) {
	...
	public function getList($keyword = '', &$total, $page, $limit = 50) {
        try {
            ...
            $query->add('select', 'i')->from('Blog', 'i');
	...
	public function getChilds($id_parent) {
        try {
            global $entityManager;
            return $entityManager->getRepository('Blog')->findBy(array('id_parent' => $id_parent), array('sort' => "ASC"));
        } catch (Exception $e) {
            return Helper::setMessage(array('type' => 'error', 'message' => $e->getMessage()));
        }
    }
