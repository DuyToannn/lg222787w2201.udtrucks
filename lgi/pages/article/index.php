...
if(isset($_GET['id']) && $_GET['id'] >= 0) require_once(__DIR__.'/item.php'); else { 
	...
	$blog = -1; if(isset($_GET['blog']) && $_GET['blog'] > -1) $blog = $_GET['blog'];
	$this->total = 0;
	$this->limit = 50;
	$this->pagina = 1; if(!empty($_GET['p']) && $_GET['p'] > 1) $this->pagina = $_GET['p'];
	$this->list = $this->model->article->getList($search, $this->total, $this->pagina, $this->limit, $status, $blog);
...
<div class="block"><form method="post">
	<table class="list-data">
		<tr><th width="10">ID</th><th align="left">Bài viết</th><th width="100">Lượt xem</th><th width="100">Cập nhật</th><th width="100">Tiêu điểm</th><th width="100">Nổi bật</th><th width="100">Trạng thái</th><th width="110"><a href="?page=article&id=0">THÊM MỚI</a></th></tr>
		<?php foreach($this->list as $item) : ?>
		<tr>
			<td><?php echo $item->id; ?></td>
			<td class="name"><a href="?page=article&id=<?php echo $item->id; ?>"><?php echo $item->title; ?></a></td>
			<td align="center"><?php echo $item->view; ?></td>
			<td align="center"><?php echo date('H:i d/m',$item->updated); ?></td>
			
			<td align="center"><button type="submit" name="hot" value="<?php echo $item->id; ?>" onclick="return confirm('Bạn đã chắc chắn!?')">
			<i class="<?php if($item->hot) echo 'ion-android-star'; else echo 'ion-android-star-outline'; ?>"></i></button></td>
			...
			<td align="center">
				<a href="?page=article&id=<?php echo $item->id; ?>"><i class="ion-android-open"></i></a>
	...
	<form id="frm-pagination" method="get" class="fr">
		...
		?>
		<select name="blog" onchange="$('#frm-pagination').submit();">
			<option value="-1">Tất cả danh mục</option>
			<option value="0" <?php if(isset($_GET['blog']) && $_GET['blog'] == 0) echo 'selected'; ?>>Không có danh mục</option>
			<?php foreach($blogs as $blog){ if(isset($_GET['blog']) && $_GET['blog'] == $blog->id) echo '<option value="'.$blog->id.'" selected>'.$blog->prefix.$blog->name.'</option>';
		...
		<select name="status" onchange="$('#frm-pagination').submit();">
			...
			<option value="3" <?php if(isset($_GET['status']) && $_GET['status'] == 3) echo 'selected'; ?>>Tiêu điểm</option>
		</select>
	</form>
</div>
<?php } ?>