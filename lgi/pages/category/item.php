<?php
	$this->list = array();
	$this->model->category->getParent($this->list);
	if(!empty($_GET['id'])) $this->item = $this->model->category->getOne($_GET['id']);
?>
<div class="block">
	<form method="post" class="frm-post" action="/<?php echo HOME; ?>?page=category">
		<div class="row">
			<div class="col col-6">
				...
				<div class="ipg"><label>Trạng thái</label>
					...
					<span><input type="checkbox" name="service" value="1" <?php if(!empty($this->item->service))  echo 'checked'; ?>>Dịch vụ</span>
				</div>
			...
			<div class="col col-6">
				...
				<input type="text" name="banner" value="<?php echo $this->item->banner; ?>"><a class="btn-save" href="javascript:void(0);" onclick="openFM('banner')">Chọn</a></div>
