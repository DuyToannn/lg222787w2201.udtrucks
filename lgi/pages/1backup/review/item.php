<?php 
$this->item = $this->model->review->getOne($_GET["id"]);
...
<div class="block">
	<form class="row frm-post" method="post">
		<div class="col col-6">
			...
			<div class="ipg"><label>Đánh giá</label><input readonly value="<?php echo $this->item->rate; ?>" required></div>
		</div>
		<div class="col col-6">
			...
			<div class="ipg field"><label>Đánh giá trên</label><input value="<?=$name?>" readonly><a class="btn-save" href="?page=<?=$type?>&id=<?=$this->item->code?>">Xem</a></div>
		...
		<div class="col col-12">
			<label class="forTextarea">Nội dung đánh giá</label>
			<textarea readonly disabled><?php echo $this->item->content; ?></textarea>
			<label class="forTextarea">Phản hồi</label>
			<textarea name="review"><?php echo $this->item->review; ?></textarea>
			<button class="btn-submit" type="submit" name="task" value="delete" onclick="return confirm('Bạn chắc chắn muốn xóa đánh giá!?')">Xóa đánh giá</button>
